define(["events", "apiclient", "appStorage", "credentialprovider", "apphost"], function (events, apiClientFactory, appStorage, credentialProvider, apphost) {
    ...
    function mergeServers(credentialProvider, list1, list2) {
        for (var i__q = 0, length = list2.length; i__q < length; i__q++) {
            credentialProvider.addOrUpdateServer(list1, list2[i__q]);
        }
    ...
    function compareVersions(a__w, b__e) {
        a__w = a__w.split(".");
        b__e = b__e.split(".");

        for (var i__r = 0, length = Math.max(a__w.length, b__e.length); i__r < length; i__r++) {
            var aVal = parseInt(a__w[i__r] || "0");
            var bVal = parseInt(b__e[i__r] || "0");
    ...
    var ConnectionManager = function (credentialProvider, appName, appVersion, deviceName, deviceId, capabilities, devicePixelRatio) {
        ...
        function onAuthenticated(apiClient, result, options, saveCredentials) {
            ...
            var servers = credentials.Servers.filter(function (s__t) {
                return s__t.Id === result.ServerId;
            });
        ...
        function getConnectServers(credentials) {
            ...
            if (credentials.ConnectAccessToken && credentials.ConnectUserId) {
                return ajax({
                    ...
                    return servers.map(function (i__y) {
                        return {
                            ExchangeToken: i__y.AccessKey,
                            ConnectServerId: i__y.Id,
                            Id: i__y.SystemId,
                            Name: i__y.Name,
                            RemoteAddress: i__y.Url,
                            LocalAddress: i__y.LocalAddress,
                            UserLinkType: "guest" === (i__y.UserType || "").toLowerCase() ? "Guest" : "LinkedUser"
                        };
                    });
                }, function () {
                    return credentials.Servers.slice(0).filter(function (s__u) {
                        return s__u.ExchangeToken;
                    });
                });
            }
        ...
        self.getServerInfo = function (id) {
            return credentialProvider.credentials().Servers.filter(function (s__i) {
                return s__i.Id === id;
            })[0];
        };
        ...
        self.getLastUsedServer = function () {
            ...
            servers.sort(function (a__o, b__p) {
                return (b__p.DateLastAccessed || 0) - (a__o.DateLastAccessed || 0);
            });
        ...
        self.addApiClient = function (apiClient) {
            ...
            var existingServers = credentialProvider.credentials().Servers.filter(function (s__a) {
                return stringEqualsIgnoreCase(s__a.ManualAddress, apiClient.serverAddress()) || stringEqualsIgnoreCase(s__a.LocalAddress, apiClient.serverAddress()) || stringEqualsIgnoreCase(s__a.RemoteAddress, apiClient.serverAddress());
            });
        ...
        self.getOrCreateApiClient = function (serverId) {
            var credentials = credentialProvider.credentials();
            var servers = credentials.Servers.filter(function (s__s) {
                return stringEqualsIgnoreCase(s__s.Id, serverId);
            });
            ...
            return new Promise(function (resolve, reject) {
                ...
                function onEnsureConnectUserDone() {
                    if (apiClient && apiClient.getCurrentUserId()) {
                        apiClient.getCurrentUser().then(function (u__f) {
                            localUser = u__f;
                            onLocalUserDone();
                        }, onLocalUserDone);
                    } else {
        ...
        self.logout = function () {
            ...
            for (var i__g = 0, length = self._apiClients.length; i__g < length; i__g++) {
                var apiClient = self._apiClients[i__g];
            ...
            return Promise.all(promises).then(function () {
                var credentials = credentialProvider.credentials();
                var servers = credentials.Servers.filter(function (u__j) {
                    return "Guest" !== u__j.UserLinkType;
                });

                for (var j__h = 0, numServers = servers.length; j__h < numServers; j__h++) {
                    var server = servers[j__h];
                    server.UserId = null;
                    server.AccessToken = null;
        ...
        self.getSavedServers = function () {
            ...
            servers.sort(function (a__k, b__l) {
                return (b__l.DateLastAccessed || 0) - (a__k.DateLastAccessed || 0);
            });
            return servers;
        };
        ...
        self.getAvailableServers = function () {
            ...
            return Promise.all([getConnectServers(credentials), findServers()]).then(function (responses) {
                ...
                servers.sort(function (a__z, b__x) {
                    return (b__x.DateLastAccessed || 0) - (a__z.DateLastAccessed || 0);
                });
                credentials.Servers = servers;
                credentialProvider.credentials(credentials);
                return servers;
        ...
        self.deleteServer = function (serverId) {
            ...
            var server = credentialProvider.credentials().Servers.filter(function (s__c) {
                return s__c.Id === serverId;
            });
            server = server.length ? server[0] : null;
            return new Promise(function (resolve, reject) {
                function onDone() {
                    var credentials = credentialProvider.credentials();
                    credentials.Servers = credentials.Servers.filter(function (s__v) {
                        return s__v.Id !== serverId;
                    });
                    credentialProvider.credentials(credentials);
                    resolve();
                }
    ...
    ConnectionManager.prototype.getApiClients = function () {
        ...
        for (var i__b = 0, length = servers.length; i__b < length; i__b++) {
            var server = servers[i__b];
    ...
    ConnectionManager.prototype.getApiClient = function (item) {
        ...
        return this._apiClients.filter(function (a__n) {
            var serverInfo = a__n.serverInfo();
